...
Try {
    $DisabledSystemsOU = Get-ADOrganizationalUnit -Identity $DN -ErrorAction Stop
    ...
    Try {
        $DisabledSystemsOU = New-ADOrganizationalUnit -Name 'Disabled Systems' -Path $ComputersOU.DistinguishedName -PassThru -ErrorAction Stop
